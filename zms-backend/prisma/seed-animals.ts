...
// Comprehensive animal seed data with 60 animals
const animalData = [
  // Big Cats (10 animals)
  ...
  // Additional Animals to reach 60
  { species: 'Amur Leopard', gender: 'MALE', isChild: false, age: 9, weight: 75.3 },
  { species: 'Amur Leopard', gender: 'FEMALE', isChild: false, age: 7, weight: 55.8 },
  { species: 'Snow Monkey', gender: 'MALE', isChild: false, age: 12, weight: 14.2 },
  { species: 'Snow Monkey', gender: 'FEMALE', isChild: false, age: 10, weight: 11.5 },
  { species: 'Meerkat', gender: 'MALE', isChild: false, age: 3, weight: 0.9 },
  { species: 'Meerkat', gender: 'FEMALE', isChild: false, age: 2, weight: 0.8 },
  { species: 'Meerkat', gender: 'MALE', isChild: true, age: 1, weight: 0.3 },
  { species: 'Red Panda', gender: 'MALE', isChild: false, age: 6, weight: 5.8 },
  { species: 'Red Panda', gender: 'FEMALE', isChild: false, age: 4, weight: 4.2 },
  { species: 'Sloth', gender: 'MALE', isChild: false, age: 8, weight: 7.5 },
  { species: 'Sloth', gender: 'FEMALE', isChild: false, age: 6, weight: 6.2 },
];
...
async function seedAnimals() {
  ...
  if (existingAnimals > 0) {
    ...
    console.log('🔄 Clearing existing animals and reseeding...');
    
    // Delete existing animals
    await prisma.animal.deleteMany({});
    console.log('🗑️  Existing animals cleared');
  }

  // Create animals
  console.log(`🌱 Creating ${animalData.length} animals...`);
  ...
  const ageGroups = await prisma.$queryRaw`
    SELECT 
      CASE 
        WHEN age < 2 THEN 'Baby (0-1 years)'
        WHEN age < 5 THEN 'Young (2-4 years)'
        WHEN age < 15 THEN 'Adult (5-14 years)'
        ELSE 'Senior (15+ years)'
      END as age_group,
      COUNT(*) as count
    FROM "Animal"
    GROUP BY age_group
  ...
  console.log('\n📈 Age Distribution:');
  (ageGroups as any[]).forEach(group => {
    console.log(`${group.age_group}: ${group.count}`);
  });

  console.log('\n🎉 Animal seeding completed successfully!');
}

seedAnimals()
  .catch((e) => {
    console.error('❌ Error during animal seeding:', e);
    process.exit(1);
  })
  .finally(async () => {
